        ...
        <div className="overflow-x-auto">
          <table className="w-full text-xs sm:text-sm text-left">
            <thead className="bg-slate-50 text-slate-600 font-medium border-b border-slate-200">
              <tr>
                <th className="px-3 py-3 sm:px-4 sm:py-4">Credit Note #</th>
            ...
            <tbody className="divide-y divide-slate-100 text-slate-700">
              {loading ? (
                ...
                <tr><td colSpan={5} className="px-4 py-4 text-center text-slate-400">No credit notes found.</td></tr>
              ) : (
                data.map((item: any) => (
                  <tr key={item._id} className="hover:bg-slate-50 transition-colors">
                    <td className="px-4 py-4 font-medium text-slate-900">{item.creditNoteNumber}</td>
                    ...
                    <td className="px-4 py-4 font-medium">{formatCurrency(item.amount)}</td>
