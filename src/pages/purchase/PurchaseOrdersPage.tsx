        ...
        <div className="overflow-x-auto">
          <table className="w-full text-xs sm:text-sm text-left">
            <thead className="bg-slate-50 text-slate-600 font-medium border-b border-slate-200">
              <tr>
                <th className="px-3 py-3 sm:px-6 sm:py-4">PO Number</th>
                <th className="px-3 py-3 sm:px-6 sm:py-4">Supplier Name</th>
                <th className="px-3 py-3 sm:px-6 sm:py-4">Order Date</th>
                <th className="px-3 py-3 sm:px-6 sm:py-4">Expected Delivery</th>
              ...
              {loading ? (
                ...
                data.map((item: any) => (
                  <tr key={item._id} className="hover:bg-slate-50 transition-colors">
                    ...
                    <td className="px-3 py-3 sm:px-6 sm:py-4">{new Date(item.orderDate || item.createdAt).toLocaleDateString()}</td>
                    <td className="px-3 py-3 sm:px-6 sm:py-4">{item.expectedDeliveryDate ? new Date(item.expectedDeliveryDate).toLocaleDateString() : "-"}</td>
