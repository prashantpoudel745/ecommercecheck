...
import { fetchPurchaseBills } from "@/services/purchase.service";
    ...
    <div className="p-6">
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-slate-500">Manage invoices and bills from your suppliers</p>
        </div>
          ...
          <table className="w-full text-sm text-left">
            ...
            <tbody className="divide-y divide-slate-100 text-slate-700">
              {loading ? (
                ...
                <tr><td colSpan={6} className="px-6 py-8 text-center text-slate-400">No purchase bills found.</td></tr>
              ) : (
                data.map((item: any) => (
                  <tr key={item._id} className="hover:bg-slate-50 transition-colors">
                    <td className="px-6 py-4 font-medium text-slate-900">{item.billNumber}</td>
